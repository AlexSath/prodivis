...
def mean_normalizer(tiff_list, norm_list, threshold, outlier_stddevs, raw_norm):
    ...
    norm_bool = get_norm_bool_idxs(norm_list, 0.7)
    ...
    for tiff, norm in zip(tiff_list, norm_list):
        ...
        norm_tiffs.append(mean_normalize(tiff, norm, norm_bool, out_dir, threshold, outlier_stddevs, raw_norm))
        count += 1
    print('')
...
def tiff_mean(normpath, norm_bool, thresh, stddevs, raw_norm):
...
def mean_normalize(tiffpath, normpath, norm_bool, out_dir, thresh, stddevs, raw_norm):
    ...
    tiffBW = (tiffBW / tiff_mean(normpath, norm_bool, thresh, stddevs, raw_norm).astype(np.float64)).astype(np.uint8) * 50
